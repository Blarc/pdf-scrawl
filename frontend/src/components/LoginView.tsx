...
export function LoginView() {
  ...
  return (
    <Surface level="base" className="flex flex-col items-center justify-center min-h-screen p-6">
      <Surface level="lowest"
               className="max-w-md w-full p-10 rounded-xl shadow-ambient border border-outline-variant border-opacity-10">
        <Typography level="display-lg" as="h2"
                    className="text-on-surface opacity-10 text-center mb-2 select-none">
          SCRAWL
        </Typography>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          {error && (
            <Typography level="body"
                        className="text-error text-xs font-medium bg-error bg-opacity-5 p-2 rounded border border-error border-opacity-10">
              {error}
            </Typography>
          )}
